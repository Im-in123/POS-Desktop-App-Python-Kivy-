...
class AdminWindow(BoxLayout):
    ...
    def add_product_fields(self):
        ...
        crud_purchase = TextInput(
            hint_text='Product Last Purchase', multiline=False)
    ...
    def add_user(self, first, last, user, pwd, des):
        ...
        if first == "" or last == "" or user == "" or pwd == "":
            self.notify.add_widget(Label(text="[color=#FF000][b]All Fields Required[/b][/color]", markup=True)
                                   )
            self.notify.open()
            Clock.schedule_once(self.killswitch, 1)
        else:
            self.users.insert_one({
                'first_name': first, 'last_name': last, 'user_name': user, 'password': pwd, 'designation': des, 'date': datetime.now()
            })
    ...
    def add_product(self, code, name, weight, stock, sold, order, purchase):
        ...
        if code == "" or name == "" or weight == "" or stock == "" or order == "":
            ...
            self.products.insert_one({
                'product_code': code, 'product_name': name, 'product_weight': weight,  'in_stock': stock, 'sold': sold, 'order': order, 'last_purchase': purchase
            })
            content.clear_widgets()
    ...
    def update_user(self, first, last, user, pwd, des):
        ...
        if user == "":
            ...
            if user == None:
                self.notify.add_widget(Label(text="[color=#FF000][b]Invalid username[/b][/color]", markup=True)
                                       )
                self.notify.open()
                Clock.schedule_once(self.killswitch, 1)
            else:
                ...
                self.users.update_one({'user_name': user['user_name']},      {'$set': {
                    'first_name': first, 'last_name': last, 'user_name': user['user_name'], 'password': pwd, 'designation': des, 'date': datetime.now()}
                })
                content.clear_widgets()
                users = self.get_users()
                userstable = DataTable(table=users)
    ...
    def update_product(self, code, name, weight, stock, sold, order, purchase):
        ...
        content = self.ids.scrn_product_contents
        if code == "":
            self.notify.add_widget(Label(text="[color=#FF000][b]Code required[/b][/color]", markup=True)
                                   )
            self.notify.open()
            Clock.schedule_once(self.killswitch, 1)
        else:
            ...
            if target_code == None:
                self.notify.add_widget(Label(text="[color=#FF000][b]Invalid code[/b][/color]", markup=True)
                                       )
                self.notify.open()
                Clock.schedule_once(self.killswitch, 1)
            else:
    ...
    def remove_user(self, user):
        ...
        if user == "":
            self.notify.add_widget(Label(text="[color=#FF000][b]Username required[/b][/color]", markup=True)
                                   )
            self.notify.open()
            Clock.schedule_once(self.killswitch, 1)
        else:
    ...
    def remove_product(self, code):
        ...
        if code == "":
            self.notify.add_widget(Label(text="[color=#FF000][b]Product Code required[/b][/color]", markup=True)
                                   )
            self.notify.open()
            Clock.schedule_once(self.killswitch, 1)
        else:
            ...
            if target_product == None:
                self.notify.add_widget(Label(text="[color=#FF000][b]Invalid Product Code[/b][/color]", markup=True)
                                       )
                self.notify.open()
                Clock.schedule_once(self.killswitch, 1)
            else:
    ...
    def view_stats(self):
        ...
        df = pd.read_csv(
            'C:\\Users\TraviisR\\Desktop\\python2022\\pos\\mongodbversion\\admin\\products_purchase.csv')
        # df = pd.read_csv(
        #     'products_purchase.csv')
